...
class ViewController: UIViewController,UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    var passConfirmedCases : String = ""
    var majorcases : String = ""
    ...
    override func viewDidLoad() {
        ...
        provincePass = "Canada"
        majorcases = "0"
        titless.text = "COVID19: \(provincePass)"
    ...
    
    {
        ...
        if let i = jsonPassData.firstIndex(where: { $0["prname"] as? String == prov && $0["date"] as? String == dateStr })
        {
            ...
            //Passing Daily Cases to major cases which is used for passing to chartscene
            var casess = (String(self.jsonPassData[i]["numtoday"] as? Int ?? 0))
            majorcases = casess
            
            if let scene = skView.scene as? ChartScene
            {
            
                scene.updateChart(values, majorcases ,passDate)
            ...
            DispatchQueue.main.async
            {
                ...
                if let scene = self.skView.scene as? ChartScene
                {
                    scene.updateChart(self.values,"0",self.passDate)
                }
    ...
    func parseJson(_ data: Data)
    {
     ...
     options:[]) as? [[String:Any]]
     {
        ...
        if let scene = skView.scene as? ChartScene
        {
            ...
            scene.updateChart(values, majorcases, dates.last ?? "")
        }
